...
# In production: yfinance / Bloomberg / NSE API
# Here: simulated with realistic covariance structure
# ─────────────────────────────────────────────────────
print("=" * 62)
...
price_data = pd.read_csv('/home/claude/simulated_prices.csv', index_col=0, parse_dates=True)
print(f"    {price_data.shape[1]} stocks × {price_data.shape[0]} trading days loaded")

# ─────────────────────────────────────────────────────
# STEP 2: MONTHLY RETURNS
# Resample daily prices to month-end → compute returns
# ─────────────────────────────────────────────────────
print("\n[2/6] Computing monthly returns...")
...
for i in range(13, N):
    ...
    momentum_signals.iloc[i] = window.sum()      # cumulative return signal
    vol_signals.iloc[i]      = window.std()      # volatility signal

# Align forward returns (what the portfolio earns NEXT month)
...
# ─────────────────────────────────────────────────────
print("\n[4/6] Constructing long-short portfolios...")
...
lowvol_ls   = factor_backtest(-vol_sig,      fwd_ret, "Low Vol L/S")   # neg: low vol = high score
...
# The exact metrics every interviewer will ask about
# ─────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────
# VISUALISATION — 6-panel professional chart
# ─────────────────────────────────────────────────────
...
ax7.plot(range(len(ic_roll)), ic_roll.values,
         color=C['mom'], lw=2, label=f'6M Rolling IC')
...
ax_style(ax7, f'🎯  Factor Information Coefficient | ICIR = {ic_series.mean()/ic_series.std():.2f}')
...
out_png = '/home/claude/project1_factor_backtest.png'
plt.savefig(out_png, dpi=150, bbox_inches='tight', facecolor='#080D18')
plt.close()
print(f"  Chart saved ✓")

# Save returns for Project 3 (Risk Dashboard will use these)
combined_ls.to_csv('/home/claude/p1_combined_returns.csv', header=True)
momentum_ls.to_csv('/home/claude/p1_momentum_returns.csv', header=True)
pd.DataFrame([mom_m, vol_m, comb_m]).to_csv('/home/claude/p1_metrics.csv', index=False)
